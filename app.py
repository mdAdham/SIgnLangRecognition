...
def main():
    ...
    video_capture, cv_fps_calc, \
        handsReader, mode = initialize_application()
    ...
    while True:
        ...
        results = handsReader.get_hands(processable_image)
...
def process_results(debug_image, finger_gesture_history, fps, keypoint_classifier, mode, number, point_history,
    ...
    for hand_landmarks, handednness in zip(results.multi_hand_landmarks, results.multi_handedness):
        ...
        point_history_list = pre_process_point_history(debug_image.width(), debug_image.height(),
                                                       point_history)
        hand_sign: KeyPointLabel = read_gesture(finger_gesture_history, keypoint_classifier, landmark_list, # static hand
                                                point_history, point_history_classifier, point_history_list,
                                                pre_processed_landmark_list)
        finger_gesture = PointHistoryLabel(Counter(finger_gesture_history).most_common()[0][0]) # dynamic finger
