...
def main():
    ...
    smart_home_entity = "."
    duration_after_start = 8  # in seconds
    ...
    if hand_sign_class == "ThumpUp":
        ...
        if not calc_finger_up(landmark_list, 6, 8):  # index finger
            if smart_home_domain == "light":
                ...
                data = {"entity_id": smart_home_entity,
                        "media_content_id": "Spiele Musik", "media_content_type": "custom"}
                post(homeassistant_api_playmedia,
                     headers=homeassistant_header, json=data)
            ...
            if smart_home_domain == "light":
                ...
                data = {"entity_id": smart_home_entity, "media_content_id":
                        "Spiele vorherige Musik", "media_content_type": "custom"}
                post(homeassistant_api_playmedia,
                     headers=homeassistant_header, json=data)
...
def draw_distance(image, length, pointCoordinates, rgb, isSet):
    if pointCoordinates[0] != 0 and pointCoordinates[1] != 0:
        cv2.circle(image, (pointCoordinates[0], pointCoordinates[1]),
                   10, (rgb[0], rgb[1], rgb[2]), cv2.FILLED)
    if pointCoordinates[2] != 0 and pointCoordinates[3] != 0:
        cv2.circle(image, (pointCoordinates[2], pointCoordinates[3]),
                   10, (rgb[0], rgb[1], rgb[2]), cv2.FILLED)
    if pointCoordinates[4] != 0 and pointCoordinates[5] != 0:
        cv2.circle(image, (pointCoordinates[4], pointCoordinates[5]),
                   10, (rgb[0], rgb[1], rgb[2]), cv2.FILLED)
    if pointCoordinates[0] != 0 and pointCoordinates[1] != 0 and pointCoordinates[2] != 0 and pointCoordinates[3] != 0:
        cv2.line(image, (pointCoordinates[0], pointCoordinates[1]), (
    ...
    if isSet == True:
        ...
        rgb = [255, 0, 0]
    cv2.rectangle(image, (50, 150), (85, 400), (rgb[0], rgb[1], rgb[2]), 3)
    cv2.rectangle(image, (50, int(lengthBar)), (85, 400),
                  (rgb[0], rgb[1], rgb[2]), cv2.FILLED)
    cv2.putText(image, f'{int(lengthPer)}%', (40, 450), cv2.FONT_HERSHEY_COMPLEX,
...
def draw_info_text(image, brect, handedness_side, hand_sign_text):
    ...
    cv2.putText(image, handedness_side, (brect[0] + 5, brect[1] - 4),
